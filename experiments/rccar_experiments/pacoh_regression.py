...
class EvalNetwork:
    ...
    def get_meta_trained_pacoh_models(self):
        ...
        for task in range(self.n_tasks):
            pacoh_models.append(self.init_and_train_pacoh_models(task))
        return pacoh_models

    def init_and_train_pacoh_models(self, task_id):
        ...
        pacoh_model = PACOH_NN_Regression(
            ...
            num_iter_meta_test=5 * self.n_samples_context,
    ...
    def eval_bnn(self):
        ...
        for task_num in range(self.n_tasks):
            context_data, test_data = self.setup_data(task_num)
            bnn = self.setup_bnn(context_data)
            bnn.fit(None, None, num_iter_fit=5*self.n_samples_context)
...
if __name__ == "__main__":
    ...
    pacoh_metrics_mean, pacoh_metrics_std = evaluator.eval_pacoh()
