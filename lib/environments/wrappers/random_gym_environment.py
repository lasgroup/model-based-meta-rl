...
class RandomGymEnvironment(AbstractEnvironment):

    def __init__(self, env_name, params, **kwargs):
        if 'mujocoMB' in params.env_group:
            env_group = importlib.import_module(f'lib.environments.random_mujocoMB_envs')
        else:
            env_group = importlib.import_module(f'lib.environments.random_mujoco_envs')
        env_cls = getattr(env_group, env_name)
        self.env = env_cls(
            random_scale_limit=params.env_random_scale_limit, **kwargs
        )
